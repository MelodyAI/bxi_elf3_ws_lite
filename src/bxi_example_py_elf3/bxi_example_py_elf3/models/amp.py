...
class HumanoidGaitPolicy:
    ...
    def __init__(self, model_onnx_path: str):
        ...
        Usage:
            ##1.初始化模型
            self.amp_policy = HumanoidGaitPolicy("path/to/model.onnx")
...
class HumanoidGaitPolicyLite:
    ...
    def __init__(self, model_onnx_path: str):
        ...
        Usage:
            ##1.初始化模型
            self.amp_policy = HumanoidGaitPolicyLite_test("path/to/model.onnx")
        ...
        self.vae_vel = np.zeros(3, dtype=np.float32) # 6维的VAE潜在速度变量
        
        self.vae_vel_x_max = np.zeros(1, dtype=np.float32) # 6维的VAE潜在速度变量
    ...
    def initialize_model(self, onnx_path):
        ...
        # print("AMP_test model init finished!!!")
        # print("########################################################################")
    ...
    def inference_step(self, q, dq, quat, omega, cmd_vel):
        ...
        if abs(self.vae_vel[0]) > self.vae_vel_x_max[0]:
            self.vae_vel_x_max[0] = max(self.vae_vel_x_max[0], abs(self.vae_vel[0]))
            print("vae_vel_x_max:", float(self.vae_vel_x_max[0]))
            # print("vae_vel:", float(self.vae_vel[0]))

        self.target_dof_pos = self.action[:29] * self.action_scale
